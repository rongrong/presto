        ...
        extends RowExpressionRewriteRuleSet
{
    ...
    private static PlanRowExpressionRewriter createRewriter(Metadata metadata, SqlParser sqlParser)
    {
        return new PlanRowExpressionRewriter()
        {
            ...
            {
                Map<NodeRef<Expression>, TypeWithName> types = analyzeCallExpressionTypes(callExpression, session, variableAllocator.getTypes());
            ...
            private Map<NodeRef<Expression>, TypeWithName> analyzeCallExpressionTypes(CallExpression callExpression, Session session, TypeProvider typeProvider)
            {
                List<LambdaExpression> lambdaExpressions = callExpression.getArguments().stream()
                        ...
                        .collect(toImmutableList());
                ImmutableMap.Builder<NodeRef<Expression>, TypeWithName> builder = ImmutableMap.<NodeRef<Expression>, TypeWithName>builder();
                if (!lambdaExpressions.isEmpty()) {
                    ...
                    for (int i = 0; i < lambdaExpressions.size(); i++) {
                        ...
                        Map<NodeRef<Expression>, TypeWithName> lambdaArgumentExpressionTypes = new HashMap<>();
                        Map<String, TypeWithName> lambdaArgumentSymbolTypes = new HashMap<>();
                        for (int j = 0; j < lambdaExpression.getArguments().size(); j++) {
                            LambdaArgumentDeclaration argument = lambdaExpression.getArguments().get(j);
                            TypeWithName type = functionType.getArgumentTypes().get(j);
                        ...
                        builder.put(NodeRef.of(lambdaExpression), new TypeWithName(functionType))
            ...
            private Map<NodeRef<Expression>, TypeWithName> analyze(Expression expression, Session session, TypeProvider typeProvider)
            {
            ...
            private RowExpression toRowExpression(Expression expression, Session session, Map<NodeRef<Expression>, TypeWithName> types)
            {
            ...
            private RowExpression removeOriginalExpression(RowExpression rowExpression, Session session, Map<NodeRef<Expression>, TypeWithName> types)
            {
