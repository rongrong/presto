        ...
        public Void visitProject(ProjectNode node, Void context)
        {
            ...
            for (Map.Entry<VariableReferenceExpression, RowExpression> entry : node.getAssignments().entrySet()) {
                ...
                if (isExpression(expression)) {
                    ...
                    Map<NodeRef<Expression>, TypeWithName> expressionTypes = getExpressionTypes(session, metadata, sqlParser, types, castToExpression(expression), emptyList(), warningCollector);
                    TypeWithName actualType = expressionTypes.get(NodeRef.of(castToExpression(expression)));
