...
public final class TwoStatesMapping
{
    private TwoStatesMapping() {}
    ...
    static {
        MAPPINGS = new ImmutableMap.Builder<List<Class<?>>, Class<? extends AccumulatorState>>()
                ...
                .put(ImmutableList.of(void.class, void.class), UnknownUnknownState.class)
    ...
    public static Class<? extends AccumulatorState> getStateClass(Class<?> firstState, Class<?> secondState)
    {
        List<Class<?>> key = ImmutableList.of(firstState, secondState);
        Class<? extends AccumulatorState> state = MAPPINGS.get(key);
        checkArgument(state != null, "Unsupported state type combination: (%s, %s)", firstState.getName(), secondState.getName());
        return state;
    }
}
